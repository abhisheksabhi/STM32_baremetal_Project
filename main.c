...
void init_systick_timer(uint32_t tick_hz)
{
    ...
    //Clear the value of SVR (SysTick reload is 24-bit, mask bits [23:0])
    *pSRVR &= ~(0x00FFFFFFu);
...
__attribute__((naked)) void init_scheduler_stack(uint32_t sched_top_of_stack)
{
    /* Naked function: only basic asm allowed, no C operands.
     * The first argument (sched_top_of_stack) arrives in R0 per ARM ABI. */
    __asm volatile(
        "MSR MSP, R0  \n"
        "BX  LR       \n"
    );
...
void init_tasks_stack(void)
{
	...
	{
		pPSP = (uint32_t*)(uintptr_t) user_tasks[i].psp_value;
		...
		*pPSP = 0xFFFFFFFD;
		...
		user_tasks[i].psp_value = (uint32_t)(uintptr_t)pPSP;
	}
